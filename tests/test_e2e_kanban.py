...
def test_pure_path_rejected():
    cmd_create('ZYQ-TEST-E2E-02', '/Users/bingsen/clawd/openclaw-sansheng-liubu/', 'Zhongshu', '中书省', '中书令')
    assert _get_task('ZYQ-TEST-E2E-02') is None, "纯路径标题应被拒绝"


# ── TEST 3: 正常标题正常创建
def test_normal_title():
    cmd_create('ZYQ-TEST-E2E-03', '调研工业数据分析大模型应用方案', 'Zhongshu', '中书省', '中书令', '皇上整理旨意')
    t = _get_task('ZYQ-TEST-E2E-03')
...
def test_flow_remark_cleaned():
    cmd_create('ZYQ-TEST-E2E-04', '调研工业数据分析大模型应用方案', 'Zhongshu', '中书省', '中书令')
    cmd_flow('ZYQ-TEST-E2E-04', '皇上', '中书省', '旨意传达：审查/Users/bingsen/clawd/xxx项目 Conversation blah')
    t = _get_task('ZYQ-TEST-E2E-04')
...
def test_short_title_rejected():
    cmd_create('ZYQ-TEST-E2E-05', '好的', 'Zhongshu', '中书省', '中书令')
    assert _get_task('ZYQ-TEST-E2E-05') is None, "短标题应被拒绝"


# ── TEST 6: 传旨前缀剥离
def test_prefix_stripped():
    cmd_create('ZYQ-TEST-E2E-06', '传旨：帮我写技术博客文章关于智能体架构', 'Zhongshu', '中书省', '中书令')
    t = _get_task('ZYQ-TEST-E2E-06')
    assert t is not None, "任务应被创建"
...
def test_state_update():
    cmd_create('ZYQ-TEST-E2E-07', '测试状态更新与组织联动功能', 'Zhongshu', '中书省', '中书令')
    cmd_state('ZYQ-TEST-E2E-07', 'Menxia', '方案提交门下省审议')
    t = _get_task('ZYQ-TEST-E2E-07')
    assert t is not None
...
# ── TEST 8: done 完成
def test_done():
    cmd_create('ZYQ-TEST-E2E-08', '测试任务完成状态标记功能', 'Zhongshu', '中书省', '中书令')
    cmd_done('ZYQ-TEST-E2E-08', '/tmp/output.md', '任务已完成')
    t = _get_task('ZYQ-TEST-E2E-08')
    assert t is not None
...
def test_done_not_overwritable():
    cmd_create('ZYQ-TEST-E2E-09', '测试已完成任务不可覆盖保护', 'Zhongshu', '中书省', '中书令')
    cmd_done('ZYQ-TEST-E2E-09', '/tmp/output.md', '任务已完成')
    cmd_create('ZYQ-TEST-E2E-09', '试图覆盖已完成的任务标题', 'Zhongshu', '中书省', '中书令')
    t = _get_task('ZYQ-TEST-E2E-09')
    assert t is not None
