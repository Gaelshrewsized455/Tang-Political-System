...
def detect_official(agent_id):
    mapping = {
        'main':    ('储君', '皇上'),        # legacy id for emperor
        'emperor':   ('储君', '皇上'),
        'zhongshu': ('中书令', '中书省'),
        'menxia':  ('侍中', '门下省'),
        'shangshu': ('尚书令', '尚书省'),
...
def main():
    ...
    try:
        ...
        # ── 过滤掉非 ZYQ 且非活跃的系统会话，防止看板噪音 ──
        # 规则: 仅保留 24小时内更新的活跃会话，且排除 cron/subagent 等纯后台任务
        filtered_tasks = []
        one_day_ago = now_ms - 24 * 3600 * 1000
        for t in tasks:
            # 始终保留 ZYQ 任务（如果有的话，虽然这里主要是 OC 任务，但以防万一）
            if str(t['id']).startswith('ZYQ'):
        ...
        # ── 保留已有的 ZYQ-* 旨意任务（不覆盖皇上下旨记录）──
        # ZYQ 任务的 now 字段由 Agent 自己通过 kanban_update.py progress 命令主动上报，
        ...
        if existing_tasks_file.exists():
            try:
                ...
                jjc_existing = [t for t in existing if str(t.get('id', '')).startswith('ZYQ')]
                
                # 去掉 tasks 里已有的 ZYQ（以防重复），再把旨意放到最前面
                tasks = [t for t in tasks if not str(t.get('id', '')).startswith('ZYQ')]
                tasks = jjc_existing + tasks
            except Exception as e:
                log.error(f'merge existing ZYQ tasks failed: {e}')
                pass
