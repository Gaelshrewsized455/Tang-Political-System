...
在 OpenClaw 中配置消息渠道（Feishu / Telegram / Signal），将 `emperor`（皇上）Agent 设为旨意入口。皇上会自动分拣闲聊与指令，指令类消息提炼标题后转发中书省。
...
# 添加飞书渠道（入口设为皇上）
openclaw channels add --type feishu --agent emperor
```
