...
业务层：帝国制度 (Imperial Governance Model)
  ├─ 分权制衡：皇上 → 皇上 → 中书 → 门下 → 尚书 → 六部
  ├─ 制度约束：不可越级、状态严格递进、门下必审议
  └─ 质量保障：可封驳反工、实时可观测、紧急可干预
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
技术层：OpenClaw多Agent编排 (Multi-Agent Orchestration)
  ├─ 状态机：9个状态（Pending → Emperor → Zhongshu → Menxia → Assigned → Doing/Next → Review → Done/Cancelled）
...
stateDiagram-v2
[*] --> Pending: 皇上下旨
Pending --> Emperor: 皇上接旨
Emperor --> Zhongshu: 皇上转交中书
...
DAY 1:
  10:00 - 皇上飞书："为三省六部编写完整自动化测试方案"
          皇上接旨。state = Emperor, org = 皇上
          自动派发 emperor agent → 处理此旨意
  
  10:30 - 皇上分拣完毕。判定为「工作旨意」（非闲聊）
          建任务 ZYQ-20260228-E2E
          flow_log 记录："皇上 → 皇上：下旨"
          state: Emperor → Zhongshu, org: 皇上 → 中书省
          自动派发 zhongshu agent
...
```json
{
  "id": "ZYQ-20260228-E2E",          // 任务全局唯一ID (ZYQ-日期-序号)
...
| 契约 | 含义 | 违反后果 |
|------|------|---------|
| **不可越级** | 皇上只能调中书，中书只能调门下/尚书，六部不能对外调用 | 超权调用被拒绝，系统自动拦截 |
| **状态单向递进** | Pending → Emperor → Zhongshu → ... → Done，不能跳过或倒退 | 只能通过 review_action(reject) 返回上一步 |
| **门下必审** | 所有中书提出的方案都要门下省审议，无法跳过 | 中书不能直接转尚书，门下必入 |
| **一旦Done无改** | 任务进入Done/Cancelled后不能再修改状态 | 若需修改需要创建新任务或取消后重新建 |
| **task_id唯一性** | ZYQ-日期-序号 全局唯一，同一天同一任务不重复建 | 看板防重，自动去重 |
...
```python
_STATE_FLOW = {
    'Pending':  ('Emperor',   '皇上',    '皇上',    '待处理旨意转交皇上分拣'),
    'Emperor':    ('Zhongshu','皇上',    '中书省',  '皇上分拣完毕，转中书省起草'),
    ...
    'Review':   ('Done',    '尚书省',  '皇上',    '全流程完成，回奏皇上转报皇上'),
}
```
...
单个任务的59条活动流（ZYQ-20260228-E2E 示例）：

```
kind    count  代表事件
────────────────────────────────────────────────
flow      10   状态转移链（Pending→Emperor→Zhongshu→...）
...
FOR EACH 任务:
  ...
  IF retryCount < maxRetry:
    ...
    - dispatch_for_state(task, new_state, trigger='emperor-scan-retry')
...
T+180:
  ...
  ✅ 阶段1：重试
  - retryCount: 0 → 1
  - dispatch_for_state('ZYQ-20260228-E2E', 'Zhongshu', trigger='emperor-scan-retry')
...
T+360 (若仍未恢复):
  ...
  ✅ 阶段2：升级
  - escalationLevel: 0 → 1
  - wake_agent('menxia', "💬 任务ZYQ-20260228-E2E停滞，中书省无反应，请介入")
  - flow_log: "升级至门下省协调"
...
T+720 (若仍未解决):
  ...
  - dispatch_for_state('ZYQ-20260228-E2E', 'Assigned', trigger='emperor-auto-rollback')
...
{
  "ok": true,
  "taskId": "ZYQ-20260228-001",
  "message": "旨意 ZYQ-20260228-001 已下达，正在派发给皇上"
}
```
...
```
请求：
GET /api/task-activity/ZYQ-20260228-E2E

响应：
{
  "ok": true,
  "taskId": "ZYQ-20260228-E2E",
  "taskMeta": {
    "title": "为三省六部编写完整自动化测试方案",
    "state": "Assigned",
    "org": "尚书省",
  ...
  "relatedAgents": ["emperor", "zhongshu", "menxia"],
  "phaseDurations": [
    {
      "phase": "皇上",
      "durationText": "30分",
...
{
  "ok": true,
  "message": "ZYQ-20260228-E2E 已推进到下一阶段 (已自动派发 Agent)",
  "oldState": "Zhongshu",
  "newState": "Menxia",
  "targetAgent": "menxia"
}
...
{
  "ok": true,
  "message": "ZYQ-20260228-E2E 已准奏 (已自动派发 Agent)",
  "state": "Assigned",
  "reviewRound": 1
}
```
...
# 说明：
# - 第一个参数：task_id
# - 第二个参数：新状态（Pending/Emperor/Zhongshu/...）
...
# 门下省想升级到皇上
try:
    result = dispatch_to_agent('emperor', '我需要皇上的指示')
except PermissionError:
    # ❌ 权限矩阵拦截
    log.error('menxia 无权调用 emperor')
```
...
curl http://127.0.0.1:7891/api/task-activity/ZYQ-20260302-001
