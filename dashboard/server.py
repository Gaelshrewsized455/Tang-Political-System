...
def handle_create_task(title, org='中书省', official='中书令', priority='normal', template_id='', params=None, target_dept=''):
    ...
    # 生成 task id: ZYQ-YYYYMMDD-NNN
    today = datetime.datetime.now().strftime('%Y%m%d')
    tasks = load_tasks()
    today_ids = [t['id'] for t in tasks if t.get('id', '').startswith(f'ZYQ-{today}-')]
    seq = 1
    if today_ids:
        nums = [int(tid.split('-')[-1]) for tid in today_ids if tid.split('-')[-1].isdigit()]
        seq = max(nums) + 1 if nums else 1
    task_id = f'ZYQ-{today}-{seq:03d}'
    # 正确流程起点：皇上 -> 皇上分拣
    ...
    dispatch_for_state(task_id, new_task, 'Emperor', trigger='imperial-edict')

    return {'ok': True, 'taskId': task_id, 'message': f'旨意 {task_id} 已下达，正在派发给皇上'}
...
_AGENT_DEPTS = [
    {'id':'main',      'label':'皇上',  'emoji':'👑', 'role':'皇上',     'rank':'天子'},
...
def _check_gateway_probe():
    """通过 WebSocket probe 检测 Gateway 是否响应。(Windows 兼容)"""
    try:
        # 尝试 WebSocket 连接
        import socket
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def _check_agent_process(agent_id):
    """检测是否有该 Agent 的 openclaw-agent 进程正在运行。(Windows 兼容)"""
    try:
        # Windows: 检查 sessions.json 是否存在且最近有更新
        sessions_file = OCLAW_HOME / 'agents' / agent_id / 'sessions' / 'sessions.json'
        if not sessions_file.exists():
            return False
        # 检查文件是否在最近 5 分钟内被修改
        ...
        return (time.time() - mtime) < 300  # 5分钟
    except Exception:
        return False
...
# 状态 → agent_id 映射
_STATE_AGENT_MAP = {
    'Emperor': 'main',     # 皇上直接由 main agent 处理
    'Zhongshu': 'zhongshu',
    'Menxia': 'menxia',
    'Assigned': 'shangshu',
...
def handle_scheduler_retry(task_id, reason=''):
    ...
    sched['lastDispatchTrigger'] = 'emperor-retry'
    _scheduler_add_flow(task, f'触发重试第{sched["retryCount"]}次：{reason or "超时未推进"}')
    task['updatedAt'] = now_iso()
    save_tasks(tasks)

    dispatch_for_state(task_id, task, state, trigger='emperor-retry')
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    task['now'] = f'↩️ 皇上调度自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = '无'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
    ...
    if snap_state not in _TERMINAL_STATES:
        dispatch_for_state(task_id, task, snap_state, trigger='emperor-rollback')
...
def handle_scheduler_scan(threshold_sec=180):
    ...
    for task in tasks:
        ...
        if retry_count < max_retry:
            ...
            sched['lastRetryAt'] = now_iso()
            sched['lastDispatchTrigger'] = 'emperor-scan-retry'
    ...
    for task_id, state in pending_retries:
        ...
        if retry_task:
            dispatch_for_state(task_id, retry_task, state, trigger='emperor-scan-retry')
    ...
    for task_id, state in pending_rollbacks:
        ...
        if rollback_task and state not in _TERMINAL_STATES:
            dispatch_for_state(task_id, rollback_task, state, trigger='emperor-auto-rollback')
...
_STATE_LABELS = {
    'Pending': '待处理', 'Emperor': '皇上', 'Zhongshu': '中书省', 'Menxia': '门下省',
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ('', '/dashboard', '/dashboard.html'):
            ...
            # SPA fallback：非 /api/ 路径返回 dashboard.html
            if not p.startswith('/api/'):
                idx = BASE / 'dashboard.html'
                if idx.exists():
                    self.send_file(idx)
                    return
            self.send_error(404)
